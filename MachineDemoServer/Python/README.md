...
### Requirements - Updated for Python asyncua V2
- Python >= Python 3.11
- Python opcua-asyncio library >= 2.0.1

Tested with 2.0.1 and Python 3.14.6 . If it will not run, use exactly that version and create an issue with error description and used library version.
...
- Install opcua-asnycio library >= 2.0.1 with 
`python -m pip install asyncua` or upgrade with `python -m pip install  --upgrade asyncua`
...
`python -m pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org --proxy=http://username:password@proxyserver:port asyncua`
